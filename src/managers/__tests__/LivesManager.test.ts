...
describe('LivesManager', () => {
  ...
  describe('display creation', () => {
    it('should create heart icons for each life', () => {
      livesManager.createDisplay(100, 50);
      expect(mockScene.add.image).toHaveBeenCalledTimes(3);
      expect(mockScene.add.image).toHaveBeenCalledWith(100, 50, 'heart-full');
      expect(mockScene.add.image).toHaveBeenCalledWith(140, 50, 'heart-full');
      expect(mockScene.add.image).toHaveBeenCalledWith(180, 50, 'heart-full');
    });

    it('should update heart display when lives change', () => {
      livesManager.createDisplay(100, 50);
      const hearts = (livesManager as any).heartIcons;
      
      livesManager.loseLife();
      
      // Should update the last heart to empty
      expect(hearts[2].setTexture).toHaveBeenCalledWith('heart-empty');
      expect(hearts[2].setAlpha).toHaveBeenCalledWith(0.5);
    });
  });
  ...
  describe('animations', () => {
    ...
    it('should not play camera effects on final life loss', () => {
      ...
      livesManager.loseLife(); // Final life
